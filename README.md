...
- Make the server port configurable from environment variables instead of hardcoding `3000`.
- Add a production-grade error envelope so all errors return one response shape.
- Add Docker and compose files for local and production parity.
- Add CI workflows for tests and linting.
- Add rate limiting for auth and upload routes.
- Add paginated filtering to more list endpoints if expected volume grows.
- Add explicit schemas or validation objects for inbound request bodies.
- Add more durable logging and tracing.
- Add retry observability and dead-letter reporting for queue failures.
- Add a richer match report history UI or API filters for JD and batch.

## Developer Guide

### Add a new API endpoint

1. Add the route in [routes](routes).
2. Add the controller in [controllers](controllers).
3. Put business logic in [services](services) when the endpoint does more than simple CRUD.
4. Wrap the handler with `asyncHandler`.
5. Return a consistent JSON response through `ApiResponse`.

### Add a new worker

1. Add a queue in [queues](queues).
2. Add a consumer in [workers](workers).
3. Make sure the worker process bootstraps database and Redis connections.
4. Keep worker state updates idempotent where possible.

### Add a new parser

1. Keep parser logic in the services layer.
2. Normalize output to the same data shape as the rest of the pipeline.
3. Add a fallback path if the new parser depends on an external service.

### Add a new model

1. Define a Mongoose schema in [models](models).
2. Keep user scoping explicit in service queries.
3. Add timestamps when the entity has a lifecycle.

### Add a new scoring rule

1. Add a new service under [services/match](services/match).
2. Return a normalized score and a human-readable explanation.
3. Update [services/match/scoring.service.js](services/match/scoring.service.js).
4. Update penalties if the new signal should influence final ranking.
5. Add or extend tests in [tests/match.test.js](tests/match.test.js).

### Add a new matching algorithm

1. Keep the matching logic isolated from controllers.
2. Reuse embeddings and cosine similarity helpers if semantic comparison is needed.
3. Ensure the output remains compatible with `MatchResult`.

## Contributing Guidelines

Code in this repository follows a service-oriented Node.js layout.

- Keep controllers thin.
- Keep parsing and ranking logic in services.
- Keep file names consistent with the current naming style.
- Preserve the existing JSON response format.
- Avoid inventing new layers unless they improve a real maintenance problem.
- Add tests when changing scoring or parsing behavior.
...
- The system is intentionally split into an API process and a worker process.
- Redis is optional for some token flows but required for queue processing.
- The parser supports both an LLM-first and regex-fallback path.
- Matching is explainable by design and stores intermediate score breakdowns.

### Important design decisions

- Resume files are stored in S3 instead of MongoDB.
- Parsing is asynchronous to prevent uploads from blocking on OCR or LLM latency.
- Embeddings are generated locally for semantic similarity and to avoid depending on a remote embedding API.
- Final scoring combines multiple signals rather than relying on one similarity metric.

### Trade-offs

- The fallback regex parser is less expressive than the LLM path, but it keeps the system functional when the API key is missing or the external call fails.
- The current error response shapes are not perfectly unified.
- The current deployment story is operationally simple but not packaged for containers or CI.

### Known limitations

- `PORT` is not currently read from the environment by the HTTP server.
- The repository includes some utility and scratch scripts that are not part of the runtime API.
- The test suite is focused on matching and parser behavior rather than full HTTP integration.
